...
public class SortTest {
    ...
    private SortByGPA[] GPASort;

    @Before
    public void setUp() {
        iDNumber = new Student[5];
        GPASort = new SortByGPA[5];
        for (int i = 0; i < iDNumber.length; i++) {
            iDNumber[i] = new Student("Student" + i, (int) (Math.random() * 15 + 1), (int) (Math.random() * 200 + 1));
            GPASort[i] = new SortByGPA("Student" + i, (int) (Math.random() * 15 + 1), (int) (Math.random() * 200 + 1));
        }
    }
    ...
    public static void quickSort(SortByGPA[] array, int low, int high) {
        if (array.length == 0)
            return;
        ...
        SortByGPA support = array[middle];
